...
G4VPhysicalVolume* A2DetPID::Construct1(G4LogicalVolume* MotherLogical,G4double Z0){
  ...
  G4double moth_rin=4.585*cm-1*mm;// changed dglazier 26/01/09
  //G4double moth_rout=fpid_rin+fpid_thick/2+1*cm;
  G4double moth_rout=5.55*cm;//brass ring
  G4double moth_z=fpid_z+flg_z-flg12_z+fpmt_z*2+fbase_z*2+10*mm;//extra 10mm to for supports
  fzpos=(fpid_z-moth_z)/2+6*mm;//zposition of centre of pid relative to mother, 3mm is for supposrt ring
  G4RotationMatrix *Mrot=new G4RotationMatrix();
  G4Tubs *PIDMother=new G4Tubs("PIDD",moth_rin,moth_rout,moth_z/2,0*deg,360*deg);
...
G4VPhysicalVolume* A2DetPID::Construct2(G4LogicalVolume* MotherLogical,G4double Z0){
  ...
  fzpos=(fpid_z-moth_z)/2+6*mm;//zposition of centre of pid relative to mother, 3mm is for support ring
  fpmtr_z=fzpos+fpid_z/2+flg_z-flg12_z+2*fpmt_z+2*fbase_z-5/2*mm;//zposition of the pmt supportring
  G4RotationMatrix *Mrot=new G4RotationMatrix();
  Mrot->rotateY(180*deg);//pid2 is positioned in opposite orientation
  G4Tubs *PIDMother=new G4Tubs("PIDD",moth_rin,moth_rout,moth_z/2,0*deg,360*deg);
  fMyLogic=new G4LogicalVolume(PIDMother,fNistManager->FindOrBuildMaterial("G4_AIR"),"PIDD");
 ...
 G4RotationMatrix** RotPMTR=new G4RotationMatrix*[fNPids];
  for(G4int i=0;i<fNPids;i++){
   // for(G4int i=0;i<1;i++){
    ...
    Rot[i]->rotateZ(270*deg-pid_angle-fpid_theta/2);
...
void A2DetPID::MakePhotomultipliers(){
  ...
  fbase_z=2.3*cm;
  G4double base_rin=0.0*cm;
  G4double base_rout=0.50*cm;
  fBASE=new G4Tubs("BASE",base_rin,base_rout,fbase_z,0*deg,360*deg);
...
void A2DetPID::MakeLightGuide(){
  ...
  G4double lg3_z=0.15*cm;//half length for G4Trd constructor!
  G4double lg3_y=(fpid_thick+2*lg12_y)/2;
  G4double lg3_xtop=0.485*cm;//also radius od tub
  ...
  fLGTU=new G4Tubs("LGTU",0.,lg4_r,lg4_z,0.,360*deg);
...
void A2DetPID::MakeSupports1(){
  ...
  //c which allows us to locate it easily into the brass tube.
   G4Cons* UPS3=new G4Cons("UPS3",4.585*cm,5.05*cm,4.585*cm,5.35*cm,3/2*mm,0*deg,360*deg);
    ...
    G4Tubs* solidPMTR=new G4Tubs("solidPMTR",41.35*mm,55.35*mm,5.*mm,0*deg,fpid_theta);
    G4Tubs* subPMTR=new G4Tubs("subPMTR",0*mm,11/2*mm,5*mm,0*deg,360*deg);
    G4double R=fpid_rin+fpid_thick/2;
    G4SubtractionSolid *PMTR=new G4SubtractionSolid("PMTR",solidPMTR,subPMTR,new G4RotationMatrix(),G4ThreeVector(R*cos(fpid_theta/2),R*sin(fpid_theta/2),0*cm));
...
void A2DetPID::MakeSupports2(){
  ...
  //c which allows us to locate it easily into the brass tube.
   G4Cons* UPS3=new G4Cons("UPS3",5.625*cm,6.09*cm,5.625*cm,6.39*cm,3/2*mm,0*deg,360*deg);
    ...
    //c Aluminium ring for holding PMTs at the downstream end of the detector.
    G4Tubs* solidPMTR=new G4Tubs("solidPMTR",54.2*mm,66.3*mm,5.*mm,0*deg,fpid_theta);
    G4Tubs* subPMTR=new G4Tubs("subPMTR",0*mm,11/2*mm,5*mm,0*deg,360*deg);
    G4double R=fpid_rin+fpid_thick/2;
    G4SubtractionSolid *PMTR=new G4SubtractionSolid("PMTR",solidPMTR,subPMTR,new G4RotationMatrix(),G4ThreeVector(R*cos(fpid_theta/2),R*sin(fpid_theta/2),0*cm));
    fPMTRLogic=new G4LogicalVolume(PMTR,fNistManager->FindOrBuildMaterial("G4_Al"),"PMTR");
